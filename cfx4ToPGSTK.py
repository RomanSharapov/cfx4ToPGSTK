...
usage = '''Usage: %prog [OPTIONS]... CFX-4.inp'''

argParser = OptionParser(usage=usage,
                         description='Simple converter from CFX-4 format to PGSTK code written in Python',
                         version='%prog 1.0')
argParser.add_option('-p', '--prefix', dest='prefix', default='prefix',
                     help='specify PREFIX name [default: -p prefix]', metavar='PREFIX')

opts, args = argParser.parse_args()

# You probably don't want to change anything below
working_dir = join(os.getcwd() + os.sep, opts.prefix)  # Current directory + prefix folder
mesh_folder = "mesh"
output_folder = "out"
default_bc = "18 Q 0"
RWK = 300000000
...
def main():
    ...
    if args[0]:
        ...
        logger.error("Please provide CFX-4.inp file")
        sys.exit(3)

    logger.info("Reading PGSTK config")

    pgs_config = PgstkConfig(opts.prefix, working_dir, mesh_folder, output_folder)
    ...
    logger.info("Reasonable number of processes (NProc) for this task should be {:d}".format(meshSize / 10000))
    ...
    for patch in range(npatch):
        ...
        # Grab patch range
        patchRange = [int(_) for _ in patch_slice.next()]
        ...
        # 1 = high i, 2 = high j, 3 = high k
        # 4 = low i, 5 = low j, 6 = low k
        patchDirection, patchBlkNo, patchLabel = [int(_) for _ in patch_slice.next()]
    ...
    for glue in range(nglue):
        masterPatchNo, slavePatchNo, dirIndex1, dirIndex2, dirIndex3, joinNumber = [int(_) for _ in
                                                                                    cfxFile.readline().split()]
        ...
        cnnFile.write("b{master_block:0{precision}d} {0} {2} {4} {1} {3} {5}\n".format(
            master_block=cfxPatchBlkNo[_index(masterPatchNo)],
            precision=pgs_config.blk_precision,
            *cnnMasterPatchRange))
        cnnFile.write("b{slave_block:0{precision}d} {0} {2} {4} {1} {3} {5}\n".format(
            slave_block=cfxPatchBlkNo[_index(slavePatchNo)],
            precision=pgs_config.blk_precision,
            *cnnSlavePatchRange))
    cnnFile.close()
    ...
    for zone in sorted(azn_patches.keys()):
        aznFile.write(
            "%Zone% {zone_name} b{zone_block:0{precision}d} {0} {1} {2} {3} {4} {5}\n".format(zone_name=zone.lower(),
